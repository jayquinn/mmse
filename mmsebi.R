...
#library(mirt)
model.4pl <- 'F1 = 1-13' 
results.4pl <- mirt(data=response, model=model.4pl, itemtype="4PL", SE=TRUE, verbose=FALSE)
coef.4pl <- coef(results.4pl, IRTpars=TRUE, simplify=TRUE)
items.4pl <- as.data.frame(coef.4pl$items)
print(items.4pl)
summary(results.4pl)
plot(results.4pl, type = 'trace', which.items = c(1:13))
plot(results.4pl, type = 'infotrace', which.items = c(1:13))
plot(results.4pl, type = 'info', theta_lim = c(-4,4), lwd=2)
plot(results.4pl, type = 'SE', theta_lim = c(-4,4), lwd=2)
plot(results.4pl, type = 'score', theta_lim = c(-4,4), lwd=2)
plot(results.4pl, type = 'itemscore', theta_lim = c(-4,4), lwd=2)
plot(results.4pl, type = 'rxx', theta_lim = c(-4,4), lwd=2)
score.4pl<-fscores(results.4pl,method = 'EAP')
hist(score.4pl)# EAP(default) MAP ML WLE EAPsum
#### CFA 점수산출####
#library(lavaan)
...
hist(score.CFA)
#### 각 점수 데이터프레임화 ####
score.frame<-cbind(score.CTT,score.CFA,score.rasch,score.4pl)
colnames(score.frame)<-c("CTT","CFA","rasch","fourpl")
head(score.frame)
...
cor(CTT,fourpl,method="spearman")
cor(CFA,rasch,method="spearman")
cor(CFA,fourpl,method="spearman")
cor(rasch,fourpl,method="spearman")
cor(CTT,CFA)
cor(CTT,rasch)
cor(CTT,fourpl)
cor(CFA,rasch)
...
cor(score.CTT,score.4pl,method="kendall")
cor(score.CFA,score.rasch,method="kendall")
cor(score.CFA,score.4pl,method="kendall")
cor(score.rasch,score.4pl,method="kendall")
cor(score.CTT,score.CFA)
cor(score.CTT,score.rasch)
cor(score.CTT,score.4pl)
cor(score.CFA,score.rasch)
cor(score.CFA,score.4pl)
cor(score.rasch,score.4pl)
